...
def naiveSoftmaxLossAndGradient(
    ...
    score = outsideVectors @ centerWordVec # (vocab size, 1)
    prob = softmax(score) # (vocab size, 1)
    loss = - np.log(prob[outsideWordIdx])
    dscores = prob
    dscores[outsideWordIdx] = dscores[outsideWordIdx] - 1
    gradCenterVec = outsideVectors.T @ dscores #（word vector length, 1）
    gradOutsideVecs = np.outer(dscores, centerWordVec) # (vocab size, word vector length)
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    #   param_grad = evaluate_grad(loss, data, params)
    ...
    o = outsideWordIdx
    v_c = centerWordVec # (d,)
    loss = -np.log(sigmoid(np.dot(U[o],v_c)))-np.sum(np.log(sigmoid(-np.dot(U[negSampleWordIndices],v_c))))
    ### Please use your implementation of sigmoid in here.
    gradCenterVec,gradOutsideVecs   = np.zeros(centerWordVec.shape), np.zeros(outsideVectors.shape)
    gradCenterVec += outsideVectors[outsideWordIdx]*(sigmoid(np.dot(outsideVectors[outsideWordIdx],centerWordVec))-1)
    gradOutsideVecs[outsideWordIdx] = centerWordVec*(sigmoid(np.dot(outsideVectors[outsideWordIdx],centerWordVec))-1)
    for i in range(K):
        gradCenterVec += outsideVectors[indices[i+1]]*(1-sigmoid(-np.dot(outsideVectors[indices[i+1]],centerWordVec)))
        gradOutsideVecs[indices[i+1]] += centerWordVec*(1-sigmoid(-np.dot(outsideVectors[indices[i+1]],centerWordVec)))
    ### END YOUR CODE
    ### Please use your implementation of sigmoid in here.
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        ...
        loss_iter, gradCenter_iter, gradOutside_iter= \
        ...
        loss += loss_iter
        gradCenterVecs[center_id] += gradCenter_iter
        gradOutsideVectors += gradOutside_iter
    ### END YOUR CODE
